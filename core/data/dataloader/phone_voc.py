...
class PVOCSegmentation(SegmentationDataset):
    """Pascal VOC Semantic Segmentation Dataset.
    ...
    def __init__(self, root='/data/zhangj/voc', split='train', mode=None, transform=None, **kwargs):
        super(PVOCSegmentation, self).__init__(root, split, mode, transform, **kwargs)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            # print('before np.unique(mask_edge)',np.unique(np.array(mask_edge)))
    ...
    def classes(self):
        """Category names."""
        return ('background', 'oil', 'scratch','stain')


if __name__ == '__main__':
    dataset = PVOCSegmentation()